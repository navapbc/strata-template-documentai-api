...
import pytest
from documentai_api.tasks.bda_invoker import main
...
def test_main_success():
    ...
    with (
        patch("documentai_api.tasks.bda_invoker.main.get_ddb_record") as mock_get_ddb_record,
        patch(
            "documentai_api.tasks.bda_invoker.main.invoke_bedrock_data_automation"
        ) as mock_invoke_bda,
        patch("documentai_api.tasks.bda_invoker.main.set_bda_processing_status_started"),
    ):
...
def test_main_skips_already_processed_files(process_status):
    ...
    with patch("documentai_api.tasks.bda_invoker.main.get_ddb_record") as mock_get_ddb_record:
...
def test_main_bypass_ddb_status_check():
    ...
    with (
        ...
        main("test-file.pdf", bypass_ddb_status_check=True)
...
def test_main_bda_invocation_fails():
    ...
    with (
        ...
        patch(
            "documentai_api.tasks.bda_invoker.main.classify_as_failed"
        ) as mock_classify_as_failed,
    ):
        mock_get_ddb_record.return_value = {"processStatus": "not_started"}
