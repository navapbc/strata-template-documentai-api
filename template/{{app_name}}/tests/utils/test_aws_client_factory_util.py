...
def test_get_documentai_region_default(clear_env_vars):
    """Test that _get_documentai_region() returns default when DOCUMENTAI_REGION not set."""
    region = AWSClientFactory._get_documentai_region()
    assert region == "us-east-1"


def test_get_documentai_region_from_env():
    """Test that _get_documentai_region() returns value from DOCUMENTAI_REGION env var."""
    with patch.dict(os.environ, {env.DOCUMENTAI_REGION: "eu-west-1"}):
        region = AWSClientFactory._get_documentai_region()
