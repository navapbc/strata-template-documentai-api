...
def test_calculate_bda_processing_times(ddb_doc_metadata_table):
    ...
    ddb_record = {
        ...
        "bdaStartedAt": bda_started_at.isoformat(),
    }
...
def test_build_completion_timing(has_bda_started_at, ddb_doc_metadata_table, mocker):
    ...
    ddb_record = {
        "fileName": "test-file",
        "createdAt": datetime(2026, 1, 1, 12, 0, 0, tzinfo=UTC).isoformat(),
    }

    if has_bda_started_at:
        ddb_record["bdaStartedAt"] = datetime(2026, 1, 1, 12, 0, 5, tzinfo=UTC).isoformat()
    ...
    if has_bda_started_at:
        assert any("bdaCompletedAt" in u for u in updates)
        assert any("processedDate" in u for u in updates)
...
def test_build_timing_updates(status, ddb_doc_metadata_table, mocker):
    ...
    if status == ProcessStatus.STARTED:
        assert "bdaStartedAt" in updates
        assert "bdaWaitTimeSeconds" in updates
        assert "bdaCompletedAt" not in updates
        assert "processedDate" not in updates
        assert values[":bdaWaitTimeSeconds"] == Decimal("10.0")
    elif status in [ProcessStatus.SUCCESS, ProcessStatus.FAILED]:
        assert "bdaCompletedAt" in updates
        assert "processedDate" in updates
        assert "bdaStartedAt" not in updates
        assert "bdaWaitTimeSeconds" not in updates
...
def test_get_user_provided_document_category(
    ...
    item = {
        "fileName": "test-file",
        "userProvidedDocumentCategory": user_provided_document_category,
    }
    ddb_doc_metadata_table.put_item(Item=item)
...
def test_get_ddb_record(ddb_doc_metadata_table):
    ...
    assert ddb_record.file_name == "test-file"
    assert ddb_record.user_provided_document_category == "income"
    assert ddb_record.process_status == "completed"
...
def test_update_ddb(status, has_timing, ddb_doc_metadata_table, mocker):
    ...
    assert item["processStatus"] == status
    assert item["v1ApiResponseJson"] == json.dumps(mock_v1.return_value)
...
def test_insert_ddb(ddb_doc_metadata_table, mocker):
    ...
    record = DocumentMetadata(
        ...
        response_json=json.dumps(internal_response.__dict__),
        ...
        document_metrics_raw=json.dumps(mock_raw_metrics.to_json_dict()),
        document_metrics_normalized=json.dumps(mock_normalized_metrics.to_json_dict()),
        overall_blur_score=Decimal("0.85"),
        created_at=now,
        updated_at=now,
    )

    ddb_util.insert_ddb(record)

    item = ddb_doc_metadata_table.get_item(Key={"fileName": object_key})["Item"]

    # base fields
    assert item["fileName"] == "test-file"
    assert item["userProvidedDocumentCategory"] == "income"
    assert item["processStatus"] == ProcessStatus.NOT_STARTED.value
    assert item["fileSizeBytes"] == 1024
    assert item["contentType"] == "application/pdf"
    assert "createdAt" in item
    assert "updatedAt" in item

    # optional fields
    assert item["pagesDetected"] == 5
    assert item["jobId"] == "job-123"
    assert item["traceId"] == "trace-456"
    assert item["isPasswordProtected"] is True
    assert item["isDocumentBlurry"] is False
    assert "responseJson" in item
    assert "documentMetricsRaw" in item
    assert "documentMetricsNormalized" in item
    assert item["overallBlurScore"] == Decimal("0.85")
